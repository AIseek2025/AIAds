...
import { TikTokSyncResult } from './tiktok.types';
...
export class TikTokSyncService {
  ...
  public init(): void {
    ...
    if (!process.env.TIKTOK_CLIENT_ID || !process.env.TIKTOK_CLIENT_SECRET) {
      logger.warn('TikTok integration disabled: missing credentials');
      return;
    }
    ...
    logger.info('TikTok sync jobs initialized', {
   ...
   * Active KOLs are those who have connected TikTok and have recent activity
   */
  ...
  /**
   * Sync active KOLs
   * Active KOLs: connected TikTok account and synced in the last 7 days
   */
  async syncActiveKols(): Promise<SyncJobStats> {
    ...
    try {
      // Find active KOLs with TikTok accounts
  ...
  /**
   * Sync all KOLs with TikTok accounts
   */
  async syncAllKols(): Promise<SyncJobStats> {
    ...
    try {
      // Find all KOLs with TikTok accounts
      const kolAccounts = await prisma.kolAccount.findMany({
        where: {
          platform: 'tiktok',
          accessToken: { not: null },
  ...
  async checkAndRefreshTokens(): Promise<void> {
    try {
      ...
      for (const account of expiringAccounts) {
        try {
          await tiktokService.syncKolData(account.kolId, {
            syncUserInfo: true,
            syncVideos: false,
  ...
  private async syncWithRetry(
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        const result = await tiktokService.syncKolData(kolId, {
          syncUserInfo: true,
   ...
   */
  async syncKol(kolId: string, fullSync: boolean = false): Promise<TikTokSyncResult> {
    try {
      // Verify KOL has TikTok account
      ...
      return await tiktokService.syncKolData(kolId, {
        syncUserInfo: true,
        syncVideos: fullSync,
  ...
  public start(): void {
    ...
    logger.info('All TikTok sync jobs started');
  }
}
