 ...
 *
 * Handles scheduled synchronization of YouTube KOL data.
...
import { YouTubeSyncResult } from './youtube.types';
...
export class YouTubeSyncService {
  ...
  public init(): void {
    ...
    if (!process.env.YOUTUBE_CLIENT_ID || !process.env.YOUTUBE_CLIENT_SECRET) {
      logger.warn('YouTube integration disabled: missing credentials');
      return;
    }
    ...
    logger.info('YouTube sync jobs initialized', {
   ...
   * Active KOLs are those who have connected YouTube and have recent activity
   */
  ...
  /**
   * Sync active KOLs
   * Active KOLs: connected YouTube account and synced in the last 7 days
   */
  async syncActiveKols(): Promise<SyncJobStats> {
    ...
    try {
      // Find active KOLs with YouTube accounts
  ...
  /**
   * Sync all KOLs with YouTube accounts
   */
  async syncAllKols(): Promise<SyncJobStats> {
    ...
    try {
      // Find all KOLs with YouTube accounts
      const kolAccounts = await prisma.kolAccount.findMany({
        where: {
          platform: 'youtube',
          accessToken: { not: null },
  ...
  async checkAndRefreshTokens(): Promise<void> {
    try {
      ...
      for (const account of expiringAccounts) {
        try {
          await youtubeService.syncKolData(account.kolId, {
            syncChannelInfo: true,
            syncVideos: false,
            syncVideoStats: false,
          });
  ...
  private async syncWithRetry(
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        const result = await youtubeService.syncKolData(kolId, {
          syncChannelInfo: true,
          syncVideos: options?.syncVideos || false,
          syncVideoStats: false,
          maxVideos: 5,
        });
   ...
   */
  async syncKol(kolId: string, fullSync: boolean = false): Promise<YouTubeSyncResult> {
    try {
      // Verify KOL has YouTube account
      ...
      return await youtubeService.syncKolData(kolId, {
        syncChannelInfo: true,
        syncVideos: fullSync,
        syncVideoStats: fullSync,
        maxVideos: fullSync ? 30 : 5,
      });
    } catch (error) {
  ...
  public start(): void {
    ...
    logger.info('All YouTube sync jobs started');
  }
}
