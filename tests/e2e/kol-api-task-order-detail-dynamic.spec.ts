...
/** KOL 订单列表首条驱动 GET /tasks/kols/orders/:id */
test.describe('KOL API task order detail (list-driven)', () => {
  test('GET /tasks/kols/orders/:id when list has rows', async ({ request }) => {
    ...
    const list = await request.get(`${base}/api/v1/tasks/kols/orders?page=1&page_size=5`, {
    ...
    const res = await request.get(`${base}/api/v1/tasks/kols/orders/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
