...
/** 广告主 GET /api/v1/orders */
test.describe('Advertiser API orders list', () => {
  test('returns paginated orders or 404 profile', async ({ request }) => {
    ...
    const res = await request.get(`${base}/api/v1/orders?page=1&page_size=5`, {
      headers: { Authorization: `Bearer ${token}` },
    });
