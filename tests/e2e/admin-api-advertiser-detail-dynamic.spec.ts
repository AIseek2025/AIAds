...
/** 广告主列表首条驱动 GET /admin/advertisers/:id */
test.describe('Admin API advertiser detail (list-driven)', () => {
  test('GET /advertisers/:id when list has rows', async ({ request }) => {
    ...
    const res = await request.get(`${base}/api/v1/admin/advertisers/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
